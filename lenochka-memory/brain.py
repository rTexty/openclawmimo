...
# =========================================================
# 6.5. LOCAL ENTITY EXPANSION (без импорта mem.py)
# =========================================================
...
def _expand_entity_context_local(top_results, conn):
    ...
    for r in top_results:
        ...
        # Также chaos_entries → memory_id → FK
        try:
            crow = conn.execute(
    ...
    if contact_ids:
        ...
        for r in rows:
            expansion["contacts"][r["id"]] = {
                "name": r["name"], "tg_username": r["tg_username"],
                "phones": r["phones"], "company": r["company_name"], "notes": r["notes"],
            }
    ...
    if deal_ids:
        ...
        for r in rows:
            expansion["deals"][r["id"]] = {
                "amount": r["amount"], "stage": r["stage"],
                "expected_close_at": r["expected_close_at"],
                "contact_name": r["contact_name"], "lead_source": r["lead_source"],
                "notes": r["notes"],
            }
    ...
    if task_conds:
        ...
        for r in rows:
            expansion["tasks"].append({
                "description": r["description"], "due_at": r["due_at"],
    ...
    if mem_conds and source_memory_ids:
        ...
        """, mem_params + list(source_memory_ids)).fetchall()
        for r in rows:
