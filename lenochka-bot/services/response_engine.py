...
def classify_and_route_batch(texts: list[str], chat_contexts: list[str],
                              brain, crm_contexts: list[str] | None = None) -> list[dict]:
    """
    ...
    for i, text in enumerate(texts):
        ctx_str = ""
        if i < len(chat_contexts) and chat_contexts[i]:
            ctx_str += f" [чат: {chat_contexts[i][:150]}]"
        if crm_contexts and i < len(crm_contexts) and crm_contexts[i]:
            ctx_str += f" [CRM: {crm_contexts[i][:200]}]"
        numbered.append(f"{i+1}. {text}{ctx_str}")
